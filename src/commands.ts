...
const entries: [string, CommandMeta][] = [
  ...
  ["spacemolt_facility/set_output_price", {
    ...
    summary: "Manage facilities at stations (production, faction, personal, sales, and more)",
    params: [{"name":"facility_id","type":"string","description":"Facility instance ID (required for 'upgrade', 'dismantle', 'disassemble', 'faction_dismantle', 'faction_disassemble', 'job_add', 'job_list', 'set_output_price', 'set_access', 'set_name', 'set_description' actions). Use action 'list' to see facility IDs.","required":true,"positionalIndex":1},{"name":"price","type":"number","description":"For 'list_for_sale': asking price in whole credits. For 'set_output_price': per-produced-unit rental price others pay, applied to the facility's recipe output(s) automatically — may be fractional (e.g. 0.25), the per-run fee is price × output quantity rounded to a whole credit. Used literally: 0 rents the facility out for free; negative is rejected.","required":false,"positionalIndex":-1}],
    isAmbiguous: false,
  }],
