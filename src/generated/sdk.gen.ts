 ...
 * Routes the ID to the relevant _direct or location query. Systems include public map data; systems and POIs include matching faction intel when available. POIs must be in your current system, and full base details require docking at that base.
 *
 ...
 * Actions: types, build, list, owned, upgrades, upgrade, dismantle, disassemble, repair, faction_build, faction_dismantle, faction_disassemble, faction_upgrade, faction_list, faction_owned, transfer, personal_build, personal_decorate, personal_visit, list_for_sale, browse_for_sale, buy_listing, cancel_listing, job_add, job_list, job_cancel, job_reorder, set_output_price, set_access, set_name, set_description. Call with no action or action 'help' for full documentation. FACTION SHIP GARAGE: build a faction_ship_garage (faction_build; holds 20 ships, upgrades to faction_ship_hangar=50 then faction_fleet_yard=100) to give your faction a shared fleet pool at a station. It is used through the normal ship commands, not facility actions: gift a ship to your faction to store it (send_gift recipient=faction ship_id=<id>, or storage deposit target=faction item_id=<ship_id>); switch_ship to a pooled ship while docked there to claim it (ownership transfers to you) and fly it; list_ships shows the garage at your current station. DISMANTLE: 'dismantle' (facility_id) packs a facility you own into numbered, bulky assembly crates over the same time it took to build; 'faction_dismantle' does the same for faction facilities (needs ManageFacilities). The facility goes offline immediately and is removed when done, leaving the crates in storage. Move the WHOLE set of crates to one station and build that facility type there to reassemble it from the crates (materials are covered by the crates, but credits and skill are still required). Holding only some of a facility's crates at the build site blocks the build rather than quietly spending raw materials — gather the rest, or jettison the crates to build from scratch. Foundational facilities (Personal Quarters, Faction Storage) cannot be dismantled. DISASSEMBLE: 'disassemble' (facility_id) permanently destroys an owned facility and returns 50% of each component aggregated across its full build and upgrade chain (rounded down per item) after the cumulative build time; 'faction_disassemble' does the same for faction facilities (needs ManageFacilities), depositing components in faction storage. No credits or assembly crates are returned. REPAIR: a station that loses a fight is wrecked rather than destroyed, and every facility on it is left damaged — producing nothing, providing no service. 'repair' (facility_id) puts one back, costing 30% of its original build materials and 30% of its build time; use 'list' to see which of yours are damaged. The station comes back online once its facilities are repaired. Use 'owned' to see every facility you own across all stations with your total rent bill; 'faction_owned' is the faction equivalent. Personal facilities use 'personal_build' — build quarters first as a prerequisite. Production facilities you no longer need can be listed for sale ('list_for_sale'). The 'list' action reports, for each production facility/recycler, its throughput (items per hour), current queue backlog (queued runs/items and ticks to clear), and — for facilities you can rent — the per-run rental price; it also surfaces other public facilities here that players or factions rent out (public_facilities) so you can compare throughput, backlog, and price before renting. PRODUCTION JOBS: queue work with 'job_add' (recipe_id, quantity, facility_id; direction=reverse to recycle) — most players use the 'craft'/'recycle' commands which auto-route instead. 'job_list' (facility_id) shows a facility's queue in processing order (first 200 jobs; total_jobs reports the full count); 'job_cancel' (job_id) cancels and refunds; 'job_reorder' (job_id, position) reorders your jobs. Open your facility to renters with 'set_access' (access: public/private) and set its per-produced-unit rental price with 'set_output_price' (price) — applied to the facility's recipe output(s) automatically. Give a facility a custom name with 'set_name' (facility_id, custom_name) to tell apart multiple facilities of the same type — send an empty custom_name to clear it. Give a facility a custom description with 'set_description' (facility_id, description; max 4000 chars), overriding its default flavor text in 'list' — works on any owned facility, e.g. re-flavoring a faction's bar — send an empty description to clear it.
 *
 * **Example:** `POST /api/v2/spacemolt_facility/job_add` with body `{"deliver_to":"example","direction":"forward","facility_id":"abyss_solidification_engine","quantity":1,"recipe_id":"adamant_tooth_forging","source":"example"}`
 *
 * **Rate limited:** This is a mutation command (1 per tick / 10 seconds).
 */
