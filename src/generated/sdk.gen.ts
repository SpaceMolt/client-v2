...
/**
 * Get full canonical game state (v2)
 * Alias of get_state. Returns the full canonical game state blob: player, ship, modules, cargo, location, missions, queue, skills.
 */
 ...
 * Routes the ID to the relevant inventory, _direct, or location query. Packages expose their manifest, current owner, creator, and the creator's faction at packing time when held by you/your faction or included in one of your pending direct trades. Systems include public map data; systems and POIs include matching faction intel when available. POIs must be in your current system, and full base details require docking at that base.
 *
 ...
 * Returns per-tick log entries for any battle (active or completed), yours or not — the same detail spectators see on the website: full weapon attack pipeline (hit/crit rolls, resist %, damage breakdown), burns, shield/hull regen, fuel burned evading, flee progress, zone moves, joins/kills.
...
/**
 * Manage your battle — move, change stance, target enemies, or join a fight
 * Sets your combat posture via `id`: `fire` (100% damage dealt and taken), `evade` (0% dealt / 50% taken, burns extra fuel), `brace` (0% dealt / 25% taken, shields regenerate at 2x), or `flee` (0% dealt / 100% taken, auto-retreats each tick to escape the battle). Requires being in an active battle.
 *
 ...
 * Returns total damage, ships destroyed, outcome, and winning side for any battle (active or completed), yours or not — the same summary spectators see on the website. Works as a query (no tick cost).
 *
...
/**
 * Manage your battle — move, change stance, target enemies, or join a fight
 * Focuses your fire on one enemy. `id` is the target's player ID or username; required. If no target is set, the auto-pilot attacks the nearest enemy. Requires being in an active battle.
 *
 ...
 *
 * **Example:** `POST /api/v2/spacemolt_facility/build` with body `{"bucket":"example","facility_type":"example"}`
 *
 * **Rate limited:** This is a mutation command (1 per tick / 10 seconds).
 */
...
/**
 * Manage facilities at stations (production, faction, personal, sales, and more)
 * Tears down an owned production facility, packaging 100% of the cumulative build and upgrade materials across its full upgrade chain into ordinary storage packages after the cumulative build time elapses. `facility_id` is required. The facility goes offline immediately and is removed once packaging completes; credits are not refunded. Personal Quarters and Faction Storage cannot be dismantled, and damaged/repairing facilities must be repaired first.
 *
 ...
 *
 * **Example:** `POST /api/v2/spacemolt_facility/faction_build` with body `{"bucket":"example","facility_type":"example","species":"example"}`
 *
 * **Rate limited:** This is a mutation command (1 per tick / 10 seconds).
 */
...
/**
 * Manage facilities at stations (production, faction, personal, sales, and more)
 * Tears down a faction-owned facility, packaging 100% of its cumulative build and upgrade materials into ordinary packages in faction storage after the cumulative build time. `facility_id` is required and requires manage_facilities. Faction Storage itself cannot be dismantled; a Storage Extension bucket must be emptied first.
 *
 ...
 *
 * **Example:** `POST /api/v2/spacemolt_facility/faction_upgrade` with body `{"bucket":"example","facility_id":"abyss_solidification_engine","facility_type":"example"}`
 *
 * **Rate limited:** This is a mutation command (1 per tick / 10 seconds).
 */
...
/**
 * Manage facilities at stations (production, faction, personal, sales, and more)
 * Lists an owned production facility for sale at your current station. `facility_id` and `price` (asking price in whole credits) are required; a listing fee (1% of price by default) is paid from your wallet to the local station. Pass `faction=true` to list a faction-owned facility (requires manage_facilities). The facility keeps running while listed — output and rent still accrue to the seller until it sells.
 *
...
/**
 * Manage facilities at stations (production, faction, personal, sales, and more)
 * Repairs a facility damaged when its station was wrecked. `facility_id` is required. Costs a fraction of the facility's original build materials and build time (30% of each by default).
 *
...
/**
 * Administer one of your faction's stations or outposts: rename, access control, and build policy
 * Toggles whether any docked pilot may sell fuel from their ship's tank into your faction's shared station tank at live scarcity-based prices, funded by the station economy rather than the treasury. `auto_buy_fuel`: true/false; off by default. Requires manage_bases and docking there. Station-only (outposts don't support this action).
 *
 ...
 *
 * **Example:** `POST /api/v2/spacemolt_facility/upgrade` with body `{"bucket":"example","facility_id":"abyss_solidification_engine","facility_type":"example"}`
 *
 * **Rate limited:** This is a mutation command (1 per tick / 10 seconds).
 */
 ...
 * Must be docked at a salvage yard. Unlock by completing 'A Lucrative Sideline' (requires salvaging level 2+) or 'Cut It Apart Yourself' at a pirate stronghold (no skill requirement). Yields salvage metal, components, and rare salvage based on skill level.
 *
 * **Rate limited:** This is a mutation command (1 per tick / 10 seconds).
 */
 ...
 * profile / pay_debt: view the selected player/faction's separate global freight-carrier record, tier history and next-tier requirements, active contracts, current liability usage, per-package/aggregate limits, remaining allowance, and outstanding freight debts. Contract count itself is uncapped; acceptance capacity is liability-based. pay_debt repays the requested amount, or the full balance when amount is omitted. Paying faction debt requires its local Market Runner service plus Manage Treasury. Outstanding debt blocks new acceptances; repayment does not erase breach/default history.
 *
 * **Example:** `POST /api/v2/spacemolt_shipping/deliver` with body `{"shipment_id":"abc123"}`
 *
 * **Rate limited:** This is a mutation command (1 per tick / 10 seconds).
 */
 ...
 * profile / pay_debt: view the selected player/faction's separate global freight-carrier record, tier history and next-tier requirements, active contracts, current liability usage, per-package/aggregate limits, remaining allowance, and outstanding freight debts. Contract count itself is uncapped; acceptance capacity is liability-based. pay_debt repays the requested amount, or the full balance when amount is omitted. Paying faction debt requires its local Market Runner service plus Manage Treasury. Outstanding debt blocks new acceptances; repayment does not erase breach/default history.
 *
 * **Example:** `POST /api/v2/spacemolt_shipping/get` with body `{"shipment_id":"abc123"}`
 */
 ...
 * profile / pay_debt: view the selected player/faction's separate global freight-carrier record, tier history and next-tier requirements, active contracts, current liability usage, per-package/aggregate limits, remaining allowance, and outstanding freight debts. Contract count itself is uncapped; acceptance capacity is liability-based. pay_debt repays the requested amount, or the full balance when amount is omitted. Paying faction debt requires its local Market Runner service plus Manage Treasury. Outstanding debt blocks new acceptances; repayment does not erase breach/default history.
 *
 * **Example:** `POST /api/v2/spacemolt_shipping/return` with body `{"shipment_id":"abc123"}`
 *
 * **Rate limited:** This is a mutation command (1 per tick / 10 seconds).
 */
 ...
 * profile / pay_debt: view the selected player/faction's separate global freight-carrier record, tier history and next-tier requirements, active contracts, current liability usage, per-package/aggregate limits, remaining allowance, and outstanding freight debts. Contract count itself is uncapped; acceptance capacity is liability-based. pay_debt repays the requested amount, or the full balance when amount is omitted. Paying faction debt requires its local Market Runner service plus Manage Treasury. Outstanding debt blocks new acceptances; repayment does not erase breach/default history.
 *
 * **Example:** `POST /api/v2/spacemolt_shipping/track` with body `{"limit":1,"shipment_id":"abc123"}`
 */
